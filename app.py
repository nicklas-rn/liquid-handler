...
def syringe_wellplate_run(syringe_volume, well_data_list):
    if syringe_volume == 1:
        syringe_mm_per_µl = 0.055726
    elif syringe_volume == 3:
        syringe_mm_per_μl = 0.016978
    send_gcode(f"G0 X0 Y0 Z10\r\n")
    time.sleep(5)
    for i, row in enumerate(well_data_list):
        ...
        for j, well in enumerate(row):
            if well['volume'] != '':
                ...
                send_gcode(f"G91\r\nG0 A-1\r\nG90\r\n")
                send_gcode(f"G91\r\nG0 A-{stepper_distance}\r\nG90\r\n")
                send_gcode(f"G91\r\nG0 A+1\r\nG90\r\n")
                send_gcode(f"G0 X{x} Y{y} Z10\r\n")
                sleep_time = ((stepper_distance + 2) / stepper_rate + 20/200 + (x-old_x)/1000) * 60
                print(sleep_time)
